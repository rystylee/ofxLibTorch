...
void AdaIN::forward(const ofFbo& contentFbo, const ofFbo& styleFbo, const float alpha)
{
    ...
    // Content image
    at::Tensor contentTensor = ofx::libtorch::util::fboToFloatTensor(contentFbo, *mDevice.get());

    // Style image
    at::Tensor styleTensor = ofx::libtorch::util::fboToFloatTensor(styleFbo, *mDevice.get());
